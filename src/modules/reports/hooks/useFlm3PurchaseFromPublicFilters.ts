...
export interface Flm3PurchaseFromPublicFiltersState {
...
const parseView = (value: string): Flm3ReportView =>
  value === Flm3ReportViewEnum.EXTENDED
    ? Flm3ReportViewEnum.EXTENDED
    : Flm3ReportViewEnum.NORMAL;

export const useFlm3PurchaseFromPublicFilters =
  (): Flm3PurchaseFromPublicFiltersState => {
    const { user } = useAuth();
    ...
    const [view, setViewState] = useState<Flm3ReportView>(
    ...
    const setView = (nextView: Flm3ReportView) => {
...
export default useFlm3PurchaseFromPublicFilters;
