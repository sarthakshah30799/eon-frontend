...
export const CashReportTable = ({
  ...
  if (!sections.length) {
    return (
      <div className="rounded-md border border-slate-100 px-3 py-8 text-center text-[11px] text-text-secondary">
        No cash ledger movements found for the selected filters.
      </div>
    );
  }
            ...
            <table className="min-w-full text-left text-[11px]">
              ...
              <tbody>
                {section.rows.map((row, index) => (
                  <tr
                    key={`${section.accountId}-${index}-${row.transactionType}-${row.number}-${row.type}`}
                    className={
                  ...
                  >
                    {columns.map(column => {
                      const value =
                        row[column.key as keyof ICashReportRow] ?? '';
                      return (
                        <td
                          key={column.key}
