...
import type { StockRevaluationFrequency, StockRevaluationTarget } from '@/api/stockRevaluation';
...
const frequencyLabel = (value?: string) => value?.replace('_', ' ').toLowerCase().replace(/\b\w/g, letter => letter.toUpperCase()) ?? 'Not configured';
...
export const StockRevaluationView = () => {
  ...
  const toggleTarget = (target: StockRevaluationTarget, checked: boolean) => {
    setSelectedTargets(previous => checked
      ? previous.some(item => item.branchId === target.branchId && item.counterId === target.counterId) ? previous : [...previous, target]
      : previous.filter(item => item.branchId !== target.branchId || item.counterId !== target.counterId));
  };
  ...
  const downloadTemplate = async () => {
    try {
      downloadBlob(await selected.downloadTemplate(), 'stock-revaluation-template.xlsx');
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to download template');
    }
  };

  const rows = selected.reports.flatMap(report => report.items.map(item => ({ report, item })));

  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-lg font-semibold text-text-primary">Stock Revaluation</h1>
        <p className="text-xs text-text-secondary">Upload period-end rates and compare them with the counter stock holding value.</p>
      </div>
      <CardSection heading="Revaluation Input" className="space-y-4">
        <div className="grid gap-3 md:grid-cols-2">
          <div className="space-y-2 rounded-md border border-slate-200 bg-white p-3">
            <p className="text-xs font-semibold text-text-primary">Branch and Counter</p>
            ...
            {!counterOptions.length && <p className="text-xs text-text-secondary">No active counters available</p>}
            <p className="text-[11px] text-text-secondary">{isAllAccess ? 'Admin/HO can select multiple branch-counter combinations' : 'Current branch and counter only'}</p>
          </div>
          <div className="space-y-3 rounded-md border border-slate-200 bg-white p-3">
            <p className="text-xs font-semibold text-text-primary">Configured frequency</p>
            <p className="text-sm text-text-primary">{frequencyLabel(stockSetting)}</p>
            <p className="text-[11px] text-text-secondary">The uploaded date is converted to the financial-year period end.</p>
          </div>
        </div>
