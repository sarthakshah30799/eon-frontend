...
const ReceiptHeader = ({ readOnly, issuers, hoBranches, transactionDatePolicy, onHoBranchChange }: { readOnly: boolean; issuers: IPartyProfile[]; hoBranches: Array<{ id: string; code: string; name: string }>; transactionDatePolicy: ReturnType<typeof getTransactionDatePolicy>; onHoBranchChange: (value: string | string[] | null) => void }) => {
  const hoBranchOptions = useMemo(() => hoBranches.map(branch => ({ value: branch.id, label: `${branch.code} - ${branch.name}` })), [hoBranches]);

  return <CardSection heading="Receipt Stock" className="space-y-4"><div className="grid gap-3 md:grid-cols-4"><StaticFormFieldSelect name="hoBranchId" label="HO Branch" options={hoBranchOptions} disabled={readOnly || hoBranches.length <= 1} onValueChange={onHoBranchChange} /><FormFieldDatePicker name="receiptDate" label="Transaction Date" disabled={readOnly || !transactionDatePolicy.canPunchTransactions} minDate={transactionDatePolicy.minDate} maxDate={transactionDatePolicy.maxDate} /><FormFieldInput name="transactionNumber" label="Transaction Number" disabled placeholder="Generated by account numbering" /><CardIssuerProfileField readOnly={readOnly} issuers={issuers} /></div><p className="text-xs text-text-secondary">CARD product rows are filtered to Product Profile code CC. Each card row represents one card and quantity is fixed to 1.</p></CardSection>;
};
...
export const CardStockReceiptForm = ({ initialValues, readOnly = false, onSubmit }: CardStockFormProps) => {
  ...
  const selectedBranchPolicy = useQuery({
    queryKey: ['card-stock', 'transaction-date-policy', selectedHoBranchId],
    queryFn: () => transactionPoliciesApi.getPolicyContext(selectedHoBranchId),
    enabled: Boolean(selectedHoBranchId),
  });
  const selectedPolicyContext = selectedBranchPolicy.data ?? (selectedHoBranchId === activeBranchId ? policyContext : null);
