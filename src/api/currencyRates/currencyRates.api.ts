...
export const currencyRatesApi = {
  getGroups: async (search?: string): Promise<ICurrencyRateGroup[]> => {
    ...
    const res = await apiClient.get<ICurrencyRateGroup[]>(
      `/currency-rates/groups${params}`
    );
  ...
  createGroup: async (
    data: Partial<ICurrencyRateGroup> & { code: string; name: string }
  ): Promise<ICurrencyRateGroup> => {
    const res = await apiClient.post<ICurrencyRateGroup>(
      '/currency-rates/groups',
      data
    );
    if (res.error) throw new Error(res.error);
  ...
  updateGroup: async (
    id: string,
    data: Partial<ICurrencyRateGroup>
  ): Promise<ICurrencyRateGroup> => {
    const res = await apiClient.put<ICurrencyRateGroup>(
      `/currency-rates/groups/${id}`,
      data
    );
    if (res.error) throw new Error(res.error);
  ...
  createRateEntry: async (data: {
    ...
    const res = await apiClient.post<ICurrencyRate>(
  ...
  preview: async (data: {
    ...
    const res = await apiClient.post<ICurrencyRateQuote>(
  ...
  getProductCurrencyRates: async (params?: {
    productId?: string;
    currencyId?: string;
    ...
    const url = query.toString()
      ? `/currency-rates/product-rules?${query.toString()}`
      : '/currency-rates/product-rules';
  ...
  createProductCurrencyRate: async (data: {
    ...
    const res = await apiClient.post<IProductCurrencyRate>(
      '/currency-rates/product-rules',
      omitEmptyFields({
  ...
  updateProductCurrencyRate: async (
    ...
    const res = await apiClient.put<IProductCurrencyRate>(
      `/currency-rates/product-rules/${id}`,
      omitEmptyFields(payload)
    );
    if (res.error) throw new Error(res.error);
