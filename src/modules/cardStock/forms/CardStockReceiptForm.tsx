...
import { cardStockSchema } from '../schema';
...
const CardRows = ({ itemIndex, readOnly, issuers }: { itemIndex: number; readOnly: boolean; issuers: IPartyProfile[] }) => {
  ...
  const issuerLength = (selectedIssuer as (IPartyProfile & { cardNumberLength?: number }) | undefined)?.cardNumberLength;
  const cardCalculationKey = (cards ?? []).map(card => `${card.quantity ?? ''}:${card.denomination ?? ''}`).join('|');

  useEffect(() => {
    const nextCards = cards ?? [];
    nextCards.forEach((card, cardIndex) => {
      const amount = (Number(card.quantity || 0) * Number(card.denomination || 0)).toFixed(2);
    ...
    const total = nextCards.reduce((sum, card) => sum + Number(card.quantity || 0) * Number(card.denomination || 0), 0).toFixed(2);
  ...
  const columns = useMemo<TableColumnDef<{ id: string }>[]>(() => [
    {
      id: 'series',
      header: 'Series',
      cell: ({ row }) => <FormFieldInput name={`items.${itemIndex}.cards.${row.index}.series`} label="" disabled={readOnly} maxLength={6} />,
    },
    {
      id: 'quantity',
      header: 'Quantity',
      cell: ({ row }) => <FormFieldInput name={`items.${itemIndex}.cards.${row.index}.quantity`} label="" type="number" disabled maxLength={1} />,
    },
    {
      id: 'kitNumber',
      header: 'Kit Number',
    ...
    },
    {
      id: 'cardNumber',
      header: 'Card Number',
      cell: ({ row }) => <FormFieldInput name={`items.${itemIndex}.cards.${row.index}.cardNumber`} label="" disabled={readOnly} placeholder={issuerLength ? `${issuerLength} digits or mask` : 'Card number'} />,
    },
    {
      id: 'denomination',
      header: 'Denomination',
    ...
    },
    {
      id: 'expirationDate',
      header: 'Expiration',
      cell: ({ row }) => <FormFieldDatePicker name={`items.${itemIndex}.cards.${row.index}.expirationDate`} label="" disabled={readOnly} minDate={new Date()} />,
    },
    {
      id: 'actions',
      header: '',
  ...
  ], [fields.length, itemIndex, issuerLength, readOnly, remove]);
...
const ReceiptHeader = ({ readOnly, issuers, hoBranches, counters, transactionDatePolicy, onHoBranchChange, onCounterChange }: { readOnly: boolean; issuers: IPartyProfile[]; hoBranches: Array<{ id: string; code: string; name: string }>; counters: Array<{ id: string; counterNo: string; name: string }>; transactionDatePolicy: ReturnType<typeof getTransactionDatePolicy>; onHoBranchChange: (value: string | string[] | null) => void; onCounterChange: (value: string | string[] | null) => void }) => {
  const form = useFormContext<ICardStockFormValues>();
  const hoBranchOptions = useMemo(() => hoBranches.map(branch => ({ value: branch.id, label: `${branch.code} - ${branch.name}` })), [hoBranches]);
  const counterOptions = useMemo(() => counters.map(counter => ({ value: counter.id, label: `${counter.counterNo} - ${counter.name}` })), [counters]);

  return <CardSection heading="Receipt Stock" className="space-y-4"><div className="grid gap-3 md:grid-cols-5"><StaticFormFieldSelect name="hoBranchId" label="HO Branch" options={hoBranchOptions} disabled={readOnly || hoBranches.length <= 1} onValueChange={value => { form.setValue('counterId', '', { shouldValidate: true }); onHoBranchChange(value); }} /><StaticFormFieldSelect name="counterId" label="Counter" options={counterOptions} disabled={readOnly || !hoBranches.length || !counters.length} onValueChange={onCounterChange} /><FormFieldDatePicker name="receiptDate" label="Transaction Date" disabled={readOnly || !transactionDatePolicy.canPunchTransactions} minDate={transactionDatePolicy.minDate} maxDate={transactionDatePolicy.maxDate} /><FormFieldInput name="transactionNumber" label="Transaction Number" disabled placeholder="Generated by account numbering" /><CardIssuerProfileField readOnly={readOnly} issuers={issuers} /></div><p className="text-xs text-text-secondary">CARD product rows are filtered to Product Profile code CC. Each card row represents one card and quantity is fixed to 1.</p></CardSection>;
};
...
export const CardStockReceiptForm = ({ initialValues, readOnly = false, onSubmit }: CardStockFormProps) => {
  ...
  const { policyContext, activeBranchId, activeCounterId } = useAuth();
  const [selectedHoBranchId, setSelectedHoBranchId] = useState(initialValues.hoBranchId);
  const [selectedCounterId, setSelectedCounterId] = useState(initialValues.counterId);
  const { data: branches = [], isLoading: branchesLoading } = useListBranchProfiles({ activeOnly: true });
  const hoBranches = useMemo(() => branches.filter(branch => branch.isHeadOffice).map(branch => ({ id: branch.id, code: branch.code, name: branch.name })), [branches]);
  const { data: counters = [] } = useListCounterProfiles({ branchId: selectedHoBranchId, activeOnly: true }, Boolean(selectedHoBranchId));
  const selectedBranchPolicy = useQuery({
    queryKey: ['card-stock', 'transaction-date-policy', selectedHoBranchId, selectedCounterId],
    queryFn: () => transactionPoliciesApi.getPolicyContext(selectedHoBranchId, selectedCounterId),
    enabled: Boolean(selectedHoBranchId && selectedCounterId),
  });
  const selectedPolicyContext = selectedBranchPolicy.data ?? (selectedHoBranchId === activeBranchId && selectedCounterId === activeCounterId ? policyContext : null);
  ...
  return <Form<ICardStockFormValues> id="card-stock-receipt-form" defaultValues={defaultValues} resolver={yupResolver(cardStockSchema) as never} mode="onChange" onSubmit={formSubmit} onError={errors => { if (import.meta.env.DEV) console.error('[CARD STOCK] submit blocked by validation errors', errors); }} footer={{ submitLabel: 'Submit Receipt Stock', showSubmit: !readOnly, isSubmitDisabled: !readOnly && (!transactionDatePolicy.canPunchTransactions || selectedBranchPolicy.isFetching || !selectedHoBranchId || !selectedCounterId), onCancel: () => window.history.back() }}><CardStockFormDebug /><CardStockTransactionDateSync readOnly={readOnly} transactionDate={transactionDatePolicy.defaultTransactionDate} /><ReceiptHeader readOnly={readOnly} issuers={references.issuers} hoBranches={hoBranches} counters={counters} transactionDatePolicy={transactionDatePolicy} onHoBranchChange={value => { const nextBranch = typeof value === 'string' ? value : ''; setSelectedHoBranchId(nextBranch); setSelectedCounterId(''); }} onCounterChange={value => setSelectedCounterId(typeof value === 'string' ? value : '')} /><ReceiptItems readOnly={readOnly} currencies={references.currencies} products={references.products} issuers={references.issuers} /></Form>;
};
