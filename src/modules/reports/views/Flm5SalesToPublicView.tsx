    ...
    <div className="space-y-3">
      <div className="space-y-1">
        <h1 className="text-lg font-semibold tracking-tight text-text-primary">
          {FLM5_SALES_TO_PUBLIC_TEXT.title}
        </h1>
        <p className="max-w-3xl text-[11px] text-text-secondary">
          {FLM5_SALES_TO_PUBLIC_TEXT.description}
        </p>
      </div>

      <Flm5SalesToPublicFiltersSection filters={reportState.filters} />
      ...
      {reportState.filters.appliedFilters && (
        <div className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 text-[11px] text-text-secondary">
          ...
          {currentSummary.branches.length
            ...
            : FLM5_SALES_TO_PUBLIC_TEXT.all}{' '}
          | {FLM5_SALES_TO_PUBLIC_TEXT.productHeading} {currentSummary.product}
        </div>
      )}
        ...
        <section className="space-y-3 rounded-xl border border-border-primary bg-white p-3 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="space-y-0.5">
              <h2 className="text-sm font-semibold text-text-primary">
                {FLM5_SALES_TO_PUBLIC_TEXT.reportViewTitle}
              </h2>
              <p className="text-[11px] text-text-secondary">
                {FLM5_SALES_TO_PUBLIC_TEXT.reportViewDescription}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex items-center gap-1 rounded-full border border-slate-200 bg-slate-50 p-1">
                <Button
                  ...
                  size="sm"
                  variant={
                    reportState.filters.view === Flm5ReportViewEnum.NORMAL
                      ? 'default'
                      : 'outline'
                  }
                ...
                <Button
                  ...
                  size="sm"
                  variant={
                    reportState.filters.view === Flm5ReportViewEnum.EXTENDED
                      ? 'default'
                      : 'outline'
                  }
                  className="h-7 rounded-full px-3 text-[11px]"
              ...
              >
                {FLM5_SALES_TO_PUBLIC_TEXT.download}
              </Button>
            </div>
          </div>
          ...
          {reportState.reportError && (
            <div className="rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-[11px] text-rose-700">
              {FLM5_SALES_TO_PUBLIC_TEXT.loadError}
            </div>
          )}
...
export default Flm5SalesToPublicView;
