...
import { AsyncSelect, Button, Checkbox, DatePicker, Input, Table, type AsyncSelectOption, type TableColumnDef } from '@/components/ui';
...
import { useBulkSettleCards, useCardSettlements } from '../hooks';
...
export const CardSettlementListView = () => {
  ...
  const [filters, setFilters] = useState<CardStockSettlementFilters>({ status: CardStockSettlementStatus.PENDING_ISSUER_SETTLEMENT });
  ...
  const selected = rows.filter(row => selection[row.id] && row.status === CardStockSettlementStatus.PENDING_ISSUER_SETTLEMENT);
  const columns: TableColumnDef<CardStockSettlement>[] = [
    { id: 'select', header: ({ table }) => { const pendingRows = table.getRowModel().rows.filter(row => row.original.status === CardStockSettlementStatus.PENDING_ISSUER_SETTLEMENT); const allSelected = pendingRows.length > 0 && pendingRows.every(row => row.getIsSelected()); return <Checkbox aria-label="Select all pending settlements" checked={allSelected} disabled={pendingRows.length === 0} onChange={checked => table.setRowSelection(Object.fromEntries(pendingRows.map(row => [row.id, checked])))} />; }, cell: ({ row }) => <Checkbox aria-label={`Select ${row.original.series}`} checked={row.getIsSelected()} disabled={row.original.status !== CardStockSettlementStatus.PENDING_ISSUER_SETTLEMENT} onChange={row.getToggleSelectedHandler()} />, enableSorting: false },
    ...
    { accessorKey: 'saleDate', header: 'Sale Date', cell: ({ row }) => formatDateTime(`${row.original.saleDate}T00:00:00`, 'DD/MM/YYYY') },
  ...
  return <div className="space-y-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-2xl font-semibold text-text-primary">{CARD_SETTLEMENT_TEXT.title}</h1><p className="text-sm text-text-secondary">{CARD_SETTLEMENT_TEXT.description}</p></div><Button type="button" disabled={selected.length === 0} onClick={() => setModalOpen(true)}>{CARD_SETTLEMENT_TEXT.settleSelected} ({selected.length})</Button></div>
    <section className="rounded-sm border border-border-primary bg-surface-primary p-4 shadow-sm sm:p-6"><div className="mb-4 flex flex-wrap items-end gap-3"><div className="min-w-64 flex-1"><Input label="Search" value={search} onChange={event => setSearch(event.target.value)} placeholder="Card, series, issuer, currency or branch" /></div>{([undefined, CardStockSettlementStatus.PENDING_ISSUER_SETTLEMENT, CardStockSettlementStatus.ISSUER_SETTLED, CardStockSettlementStatus.CANCELLED] as const).map(value => <Button key={value ?? 'ALL'} type="button" size="sm" variant={filters.status === value ? 'default' : 'outline'} onClick={() => { setFilters(current => ({ ...current, status: value })); setSelection({}); }}>{value?.replaceAll('_', ' ') ?? 'ALL'}</Button>)}</div>
      <div className="mb-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="flex items-end"><Button type="button" variant="outline" onClick={() => { setFilters({ status: CardStockSettlementStatus.PENDING_ISSUER_SETTLEMENT }); setSearch(''); setSelection({}); }}>Clear Filters</Button></div>
