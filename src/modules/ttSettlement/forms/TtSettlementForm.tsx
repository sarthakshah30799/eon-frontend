...
import { useDealCoverReferences } from '@/modules/dealCoverRate/hooks';
...
import { useUnsettledTtSettlements } from '../hooks';
import type { TtSettlementFormValues } from '../types/ttSettlementTypes';
import { settlementAmountFrom, toFormItem } from '../utils/ttSettlementUtils';
import { TtSettlementItemPicker } from '../components';
...
const snapshotLabel = (
  snapshot: TtSettlementFormValues['issuerPartyProfileSnapshot']
) => {
...
export const TtSettlementForm = ({
  ...
  const isIssuerKind = isHo || kind === TtSettlementDocumentKind.HO_ISSUER;
  ...
  const canLoadUnsettled =
    ...
    !readOnly;
  const unsettled = useUnsettledTtSettlements(
    {
      kind: isIssuerKind
    ...
    <div className="space-y-6">
      ...
      >
        <div className="grid gap-4 xl:grid-cols-4">
          ...
          <FormFieldSelect
            name="issuerPartyProfileId"
            label={TT_SETTLEMENT_TEXT.issuer}
            placeholder={TT_SETTLEMENT_TEXT.selectIssuer}
          ...
          <FormFieldSelect
            name="currencyId"
            label={TT_SETTLEMENT_TEXT.currency}
            placeholder={TT_SETTLEMENT_TEXT.selectCurrency}
          ...
          {isIssuerKind ? (
            <FormFieldSelect
              name="hoBranchId"
              label={TT_SETTLEMENT_TEXT.hoBranch}
              placeholder={TT_SETTLEMENT_TEXT.selectHoBranch}
          ...
          <FormFieldDatePicker
            ...
            label={TT_SETTLEMENT_TEXT.transactionDate}
          ...
          <FormFieldInput
            ...
            label={TT_SETTLEMENT_TEXT.transactionNumber}
            placeholder={TT_SETTLEMENT_TEXT.transactionNumberPlaceholder}
            readOnly
            disabled
          />
          <FormFieldInput
            name="reference"
            label={TT_SETTLEMENT_TEXT.reference}
            valueTransform="none"
            disabled={readOnly}
          />
          <FormFieldInput
            name="remarks"
            label={TT_SETTLEMENT_TEXT.remarks}
            valueTransform="none"
            disabled={readOnly}
          />
        </div>
      ...
      <CardSection
        heading={TT_SETTLEMENT_TEXT.itemsHeading}
        className="space-y-4"
      >
        {!readOnly ? (
          <Button
          ...
          >
            {TT_SETTLEMENT_TEXT.selectItems}
          </Button>
        ) : null}
        {fields.map((field, index) => {
          ...
          return (
            ...
            >
              <FormFieldInput
                name={`items.${index}.dealNo`}
                label={TT_SETTLEMENT_TEXT.dealNo}
                readOnly
                disabled
              />
              <FormFieldInput
                name={`items.${index}.feAmount`}
                label={TT_SETTLEMENT_TEXT.feAmount}
                readOnly
                disabled
              />
              <FormFieldInput
                name={`items.${index}.dealRate`}
                label={TT_SETTLEMENT_TEXT.dealRate}
                readOnly
                disabled
              />
              <FormFieldInput
                name={`items.${index}.bookingRate`}
                label={TT_SETTLEMENT_TEXT.bookingRate}
                readOnly
                disabled
              />
              <FormFieldInput
                name={`items.${index}.rate`}
                label={TT_SETTLEMENT_TEXT.rate}
                valueTransform="none"
                disabled={readOnly}
                onChange={event => {
                  ...
                  form.setValue(
                    `items.${index}.amount`,
                    settlementAmountFrom(item?.feAmount ?? '0', rate),
              ...
              <FormFieldInput
                ...
                label={TT_SETTLEMENT_TEXT.amount}
                readOnly
                disabled
              />
            </div>
...
export default TtSettlementForm;
