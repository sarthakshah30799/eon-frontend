...
export const validateCardNumber = (value: string, rule: CardNumberRule = {}, requireMask = false) => {
  ...
  if (requireMask && allowMasking) {
    return isMasked
      ...
      : { valid: false as const, cardNumber, message: CARD_STOCK_VALIDATION_TEXT.mask(length) };
  }
  if (isDigits) {
  ...
  return {
    ...
    cardNumber,
    message: allowMasking ? CARD_STOCK_VALIDATION_TEXT.digitsOrMask(length) : CARD_STOCK_VALIDATION_TEXT.digits(length),
  };
};
