...
import type { Flm4PurchaseFromFfmcFiltersState } from '../hooks/useFlm4PurchaseFromFfmcFilters';
import CheckboxFilterGroup from './CheckboxFilterGroup';
import ReportDatePresetFilter from './ReportDatePresetFilter';

interface Flm4PurchaseFromFfmcFiltersSectionProps {
  filters: Flm4PurchaseFromFfmcFiltersState;
}
...
export const Flm4PurchaseFromFfmcFiltersSection = ({
  filters,
  ...
  return (
    <section className="space-y-3 rounded-xl border border-border-primary bg-white p-3 shadow-sm">
      ...
      <div className="grid gap-3 xl:grid-cols-3">
        <CheckboxFilterGroup
          heading={FLM4_PURCHASE_FROM_FFMC_TEXT.branchHeading}
        ...
        <CheckboxFilterGroup
          heading={FLM4_PURCHASE_FROM_FFMC_TEXT.profileHeading}
        ...
        <div className="space-y-1.5 rounded-md border border-slate-200 bg-white p-2 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <div className="text-[10px] font-semibold uppercase tracking-wider text-text-tertiary">
              {FLM4_PURCHASE_FROM_FFMC_TEXT.productHeading}
            </div>
                  ...
                  >
                    ...
                    {filters.productId === option.id ? (
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-primary-600">
                        {FLM4_PURCHASE_FROM_FFMC_TEXT.selected}
                      </span>
                    ) : null}
                  </Dropdown.Item>
                ))}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-2">
        <div className="text-[11px] text-text-secondary">
          {FLM4_PURCHASE_FROM_FFMC_TEXT.selectProductHint}
        </div>
        <div className="flex gap-2">
          ...
          >
            {FLM4_PURCHASE_FROM_FFMC_TEXT.reset}
          </Button>
          <Button
            type="button"
          ...
          >
            {FLM4_PURCHASE_FROM_FFMC_TEXT.viewReport}
          </Button>
        </div>
      </div>
    </section>
...
export default Flm4PurchaseFromFfmcFiltersSection;
