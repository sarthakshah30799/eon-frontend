...
export const CardTransferListView = () => {
  ...
  const { data: requests = [], isLoading, error } = useListCardTransfers({ status: status === 'ALL' ? undefined : status, search: search.trim() || undefined });
  ...
  const columns = useMemo<TableColumnDef<CardTransferRequest>[]>(() => [
    ...
    { accessorKey: 'sourceBranchId', header: 'Source HO Branch', cell: ({ row }) => row.original.sourceBranch?.name ?? row.original.sourceBranchId },
    { accessorKey: 'destinationBranchId', header: 'Destination Branch', cell: ({ row }) => row.original.destinationBranch?.name ?? row.original.destinationBranchId },
