...
interface IAllocationRow {
  ...
  bookId: string; // original checkbook id
  requestNo: string;
  requestDate: string;
...
export const CheckBookAllocationPage = () => {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      ...
      try {
        ...
        const data = await checkbookApi.getCashiers(activeBranchId);
        setCashiers(data);
      } catch (err: any) {
  ...
  const handleProcess = async () => {
    ...
    if (isNaN(fromVal) || isNaN(toVal) || fromVal < 1 || toVal < fromVal) {
      toast.error('Please enter a valid Checkbook range (From <= To).');
      return;
    }
    ...
    try {
      ...
      const data = await checkbookApi.findAll(activeBranchId, 'Approved');
      ...
      const allocations = matchedIds.length > 0 
        ? await checkbookApi.getAllocations(matchedIds)
        : [];
      ...
      matched.forEach(book => {
        ...
        for (let i = start; i <= end; i++) {
          ...
          const existing = allocations.find(
            a => a.checkBookId === book.id && a.bookNo === i
          );
      ...
      if (generatedRows.length === 0) {
        toast.error('No matching approved checkbooks found in the specified range.');
      } else {
        toast.success(`Found ${generatedRows.length} checkbooks.`);
      }
    } catch (err: any) {
  ...
  const handleSaveAllocation = async () => {
    ...
    try {
      ...
      await checkbookApi.saveAllocations(payload);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-1.5 border-b border-slate-200 pb-5">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Checkbook Cashier Allocation</h1>
        <p className="text-sm text-slate-500">
          Allocate individual checkbooks to cashiers at your branch.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 align-end">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">Check Book No. From</label>
            <input
              type="number"
              min="1"
              value={bookNoFromStr}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">Check Book No. To</label>
            <input
              type="number"
              min="1"
              value={bookNoToStr}
...
export default CheckBookAllocationPage;
