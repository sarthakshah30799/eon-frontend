...
const CardRows = ({
  ...
  const { fields, append, remove } = useFieldArray({
    ...
    name: `items.${itemIndex}.cards` as never,
  });
  const cards = useWatch({
    control: form.control,
    name: `items.${itemIndex}.cards` as never,
  }) as ICardStockFormCard[] | undefined;
  const issuerId = useWatch({
    control: form.control,
    name: `items.${itemIndex}.issuerPartyProfileId` as never,
  }) as string;
  const selectedIssuer = issuers.find(issuer => issuer.id === issuerId);
  const issuerLength = resolveCardNumberLength(
  ...
  useEffect(() => {
    const nextCards =
      (form.getValues(`items.${itemIndex}.cards` as never) as unknown as
        | ICardStockFormCard[]
        | undefined) ?? [];
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <FormFieldInput
            name={`items.${itemIndex}.cards.${row.index}.series`}
            label=""
            disabled={readOnly}
            maxLength={4}
          />
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <FormFieldInput
            name={`items.${itemIndex}.cards.${row.index}.kitNumber`}
            label=""
            disabled={readOnly}
          />
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <FormFieldInput
            name={`items.${itemIndex}.cards.${row.index}.cardNumber`}
            label=""
            disabled={readOnly}
            placeholder={
              allowMasking
                ? `${issuerLength} digits or mask`
                : `${issuerLength} digits`
            }
          />
        ),
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <FormFieldInput
            name={`items.${itemIndex}.cards.${row.index}.amount`}
            label=""
            type="number"
            disabled
          />
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <FormFieldDatePicker
            name={`items.${itemIndex}.cards.${row.index}.expirationDate`}
            label=""
  ...
  return (
    <div className="space-y-3 rounded-md border border-border-secondary bg-surface-secondary/40 p-3">
...
const ReceiptItems = ({
  ...
  const total = (items ?? [])
    ...
    .toFixed(2);
  useEffect(() => {
    if (form.getValues('totalFeAmount') !== total)
      form.setValue('totalFeAmount', total, { shouldValidate: true });
  }, [form, total]);

  return (
    <CardSection heading="Transaction Items" className="space-y-4">
      <div className="space-y-5">
        {fields.map((field, itemIndex) => {
          ...
          return (
            <div
              key={field.id}
              className="space-y-4 rounded-md border border-border-primary p-3"
            >
              <div className="overflow-x-auto">
                <div className="grid min-w-[1100px] grid-cols-5 gap-3">
                  ...
                  />
                  <StaticFormFieldSelect
                    key={`issuer-${itemIndex}-${product?.id ?? 'none'}`}
                    name={`items.${itemIndex}.issuerPartyProfileId`}
                    label="Issuer"
                    options={issuerOptions}
                    disabled={readOnly || !product}
                  />
                  <FormFieldInput
                    name={`items.${itemIndex}.feAmount`}
                    label="FE Amount"
                    type="number"
                    disabled
                  />
              ...
              {!readOnly ? (
                <CardStockUploadSection
                  itemIndex={itemIndex}
                  readOnly={readOnly}
                  issuer={issuers.find(
                    issuer => issuer.id === item?.issuerPartyProfileId
                  )}
                />
              ) : null}
              <CardRows
  ...
  return (
    <CardSection heading="Receipt Stock" className="space-y-4">
      ...
      <p className="text-xs text-text-secondary">
        ...
        under only-stocking currencies with product allowed CM. Each card row
        represents one card.
      </p>
    </CardSection>
  );
...
export const CardStockReceiptForm = ({
  initialValues,
  readOnly = false,
  footerActions,
  onSubmit,
  ...
  return (
    <Form<ICardStockFormValues>
      id="card-stock-receipt-form"
      defaultValues={defaultValues}
      resolver={yupResolver(cardStockSchema) as never}
      mode="onChange"
      onSubmit={formSubmit}
      onError={errors => {
