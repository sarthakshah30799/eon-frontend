...
export const useCardUnsettledReport = () => {
  ...
  const reportQuery = useQuery<ICardSettlementReportResponse>({
    queryKey: ['card-unsettled-report', filters.appliedFilters],
    enabled: Boolean(filters.appliedFilters),
    queryFn: () => reportsApi.getCardUnsettledReport(requestParams),
  });
  ...
  const downloadReport = useCallback(async () => {
    ...
    const payload = await reportsApi.downloadCardUnsettledReport(
      requestParams,
      exportFormat,
    );
    downloadBlob(payload.blob, payload.filename || 'card-unsettled-report.xlsx');
...
export default useCardUnsettledReport;
