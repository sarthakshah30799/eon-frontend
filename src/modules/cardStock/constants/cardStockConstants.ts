...
export const CARD_STOCK_UPLOAD_TEXT = {
  ...
  helperText:
    'Upload kit numbers and card numbers for this item. CSV and Excel files are parsed on the server.',
...
export const CARD_STOCK_VALIDATION_TEXT = {
  ...
  digitsOrMask: (length: number) =>
    `Card number must be ${length} digits or a ${length}-character mask`,
} as const;
