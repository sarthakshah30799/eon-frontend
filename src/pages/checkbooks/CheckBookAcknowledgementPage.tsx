...
export const CheckBookAcknowledgementPage = () => {
  const { activeBranchId } = useAuth();
  
  // Navigation: 'list' | 'detail'
  const [view, setView] = useState<'list' | 'detail'>('list');
  const [isLoadingList, setIsLoadingList] = useState(true);
  const [dispatches, setDispatches] = useState<ICheckBook[]>([]);
  ...
  const [queryResults, setQueryResults] = useState<ICheckBook[]>([]);
  ...
  const fetchDispatches = async () => {
    ...
    try {
      ...
      const data = await checkbookApi.findAll(activeBranchId);
      setDispatches(data);
    } catch (err: any) {
  ...
  const handleSaveReviews = async () => {
    ...
    try {
      setIsSaving(true);
      await checkbookApi.bulkReview(reviewsToSubmit);
  ...
  const handleRowClick = (book: ICheckBook) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-1.5 border-b border-slate-200 pb-5">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Checkbook Status</h1>
          {view === 'detail' && (
            <button
        ...
        <p className="text-sm text-slate-500">
          Acknowledge and process checkbooks allocated to your branch.
        </p>
      </div>
              ...
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/50 text-slate-600 font-medium select-none">
                    ...
                    <th className="px-6 py-3">Check Books From-To</th>
                    <th className="px-6 py-3">Leaves/Book</th>
                    <th className="px-6 py-3">Cheque From-To</th>
                ...
                <table className="w-full text-left text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-slate-200 bg-slate-50/50 text-slate-600 font-medium select-none">
                      ...
                      <th className="px-4 py-3">Check Book No. From</th>
                      <th className="px-4 py-3">Check Book No. To</th>
                      <th className="px-4 py-3">No Of Leaf</th>
...
export default CheckBookAcknowledgementPage;
