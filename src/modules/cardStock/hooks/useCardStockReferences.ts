...
export const useCardStockReferences = () => {
  ...
  const products = useQuery({
    ...
    queryFn: async () => (await productProfileApi.getProductProfiles({ activeOnly: true })).filter(product => product.productCode.toUpperCase() === 'CC'),
  });
  const currencies = useQuery({
    queryKey: ['card-stock', 'currencies'],
    queryFn: () => currencyProfileApi.getCurrencyProfiles({ activeOnly: true }),
  });
  return { issuers: issuers.data ?? [], products: products.data ?? [], currencies: currencies.data ?? [], isLoading: issuers.isLoading || products.isLoading || currencies.isLoading };
};
