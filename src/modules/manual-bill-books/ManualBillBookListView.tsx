...
export const ManualBillBookListView = () => {
  const [books, setBooks] = useState<IManualBook[]>([]);
  ...
  const fetchBooks = async () => {
    try {
      ...
      toast.error(err.message || 'Failed to load manual book records.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await manualBillBookApi.approveOrReject(selectedBook.id, {
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex flex-col gap-1.5">
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Manual Bill Book Management</h1>
          <p className="text-sm text-slate-500">Track and manage physical carbon bill book allocations and approvals.</p>
        </div>
        <button
          type="button"
                ...
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 font-semibold uppercase">
                      ...
                      <th className="p-3 text-center">Books From-To</th>
                      <th className="p-3 text-center">Vouchers/Book</th>
                      <th className="p-3 text-center">MV From-To</th>
        ...
        >
          <form onSubmit={handleReviewSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-xs bg-slate-50 p-4 border border-slate-200 rounded-md">
              ...
              <div>
                <span className="block text-slate-400 font-semibold mb-0.5">Vouchers Per Book</span>
                <span className="text-slate-800">{selectedBook.vouchersPerBook}</span>
              </div>
              <div>
                <span className="block text-slate-400 font-semibold mb-0.5">MV Range</span>
