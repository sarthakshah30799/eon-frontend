...
export const useRecordCardTransferPrint = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      queryClient.setQueryData<CardTransferRequest>(
        cardTransferQueryKeys.detail(variables.id),
        current => {
          if (!current) return current;
          if (variables.payload.kind === 'STOCK_OUT') {
            return {
              ...current,
              sourcePrintCount: (current.sourcePrintCount ?? 0) + 1,
            };
          }
          return {
            ...current,
            destinationPrintCount: (current.destinationPrintCount ?? 0) + 1,
          };
        }
      );
      void queryClient.invalidateQueries({
        queryKey: cardTransferQueryKeys.detail(variables.id),
      });
      void queryClient.invalidateQueries({
    ...
    try {
      ...
      const printingSnapshot =
        kind === 'STOCK_OUT'
          ...
          : request.destinationBranchSnapshot;
