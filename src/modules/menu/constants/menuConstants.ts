export const MENU_TEXTS = {
  CREATE_TITLE: 'Menu Management',
  FORM_SUBTITLE:
    'Create and update dynamic sidebar menus from the admin panel.',
  SAVE_CHANGES: 'Save Changes',
  CREATE_SUCCESS: 'Menu created successfully!',
  UPDATE_SUCCESS: 'Menu updated successfully!',
  DELETE_SUCCESS: 'Menu deleted successfully!',
  CREATE_ERROR: 'Failed to create menu',
  UPDATE_ERROR: 'Failed to update menu',
  DELETE_ERROR: 'Failed to delete menu',
  EMPTY_STATE: 'No menus found yet.',
  ROOT_LABEL: 'Root menu',
} as const;
