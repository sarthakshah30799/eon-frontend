...
import {
  GenerateLedgerFiltersSection,
  GenerateLedgerTable,
} from '../components';
import { useGenerateLedger } from '../hooks';
import { GenerateLedgerLayoutEnum, ReportExportFormatEnum } from '../types';
import { summarizeReportSelection } from '../utils';

export const GenerateLedgerView = () => {
  const { user } = useAuth();
  const report = useGenerateLedger();
  ...
  const currentSummary = useMemo(() => {
    ...
    const accountTypeLabels = summarizeReportSelection(
      report.filters.accountTypeIds,
      report.filters.accountTypeOptions
    );
    ...
    return {
      ...
      layout:
        report.filters.layout === GenerateLedgerLayoutEnum.CONSOLIDATED
          ? 'Consolidated'
          : 'Branch wise',
    };
  }, [
    ...
    <div className="space-y-3">
      <div className="space-y-1">
        ...
        <p className="max-w-3xl text-[11px] text-text-secondary">
          Account posting ledger by account type and account, with branch-wise
          account groups or a flat consolidated stream, Opening/Closing rows,
          debit/credit running total, and CSV/XLSX export.
        </p>
      </div>

      <GenerateLedgerFiltersSection filters={report.filters} />
      ...
      {report.filters.appliedFilters && (
        <div className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 text-[11px] text-text-secondary">
          ...
          {currentSummary.branches.length
            ...
            : 'All'}{' '}
          | Account types{' '}
          {currentSummary.accountTypes.length
            ? currentSummary.accountTypes.join(', ')
            : 'All'}{' '}
          | Accounts{' '}
      ...
      {report.filters.appliedFilters && (
        <section className="space-y-3 rounded-xl border border-border-primary bg-white p-3 shadow-sm">
          ...
          <GenerateLedgerTable
            ...
            layout={report.reportLayout ?? report.filters.layout}
...
export default GenerateLedgerView;
