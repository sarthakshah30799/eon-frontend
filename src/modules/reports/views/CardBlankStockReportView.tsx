...
import { useCardBlankStockReport } from '../hooks';
import { ReportExportFormatEnum, ReportSortByEnum } from '../types';
import { summarizeReportSelection } from '../utils';

export const CardBlankStockReportView = () => {
  const { user } = useAuth();
  const report = useCardBlankStockReport();
  ...
  const currentSummary = useMemo(() => {
    ...
    const sortLabel =
      report.filters.sortBy === ReportSortByEnum.DATE_DESC
        ? CARD_BLANK_STOCK_REPORT_TEXT.dateDesc
        : CARD_BLANK_STOCK_REPORT_TEXT.dateAsc;
    ...
    <div className="space-y-3">
      <div className="space-y-1">
        <h1 className="text-lg font-semibold tracking-tight text-text-primary">
          {CARD_BLANK_STOCK_REPORT_TEXT.title}
        </h1>
        <p className="max-w-3xl text-[11px] text-text-secondary">
          {CARD_BLANK_STOCK_REPORT_TEXT.description}
        </p>
      </div>
      ...
      {report.filters.appliedFilters && (
        <div className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 text-[11px] text-text-secondary">
          ...
          {currentSummary.branches.length
            ...
            : CARD_BLANK_STOCK_REPORT_TEXT.all}{' '}
          | {CARD_BLANK_STOCK_REPORT_TEXT.productHeading}{' '}
          {currentSummary.products.length
            ? currentSummary.products.join(', ')
            : CARD_BLANK_STOCK_REPORT_TEXT.all}{' '}
          | {CARD_BLANK_STOCK_REPORT_TEXT.currencyHeading}{' '}
          {currentSummary.currencies.length
            ? currentSummary.currencies.join(', ')
            : CARD_BLANK_STOCK_REPORT_TEXT.all}{' '}
          | {CARD_BLANK_STOCK_REPORT_TEXT.issuerHeading}{' '}
          {currentSummary.issuers.length
            ? currentSummary.issuers.join(', ')
            : CARD_BLANK_STOCK_REPORT_TEXT.all}{' '}
          | {CARD_BLANK_STOCK_REPORT_TEXT.sortBy} {currentSummary.sortLabel}
        </div>
      )}
        ...
        <section className="space-y-3 rounded-xl border border-border-primary bg-white p-3 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="space-y-0.5">
              <h2 className="text-sm font-semibold text-text-primary">
                {CARD_BLANK_STOCK_REPORT_TEXT.reportViewTitle}
              </h2>
              <p className="text-[11px] text-text-secondary">
                {CARD_BLANK_STOCK_REPORT_TEXT.reportViewDescription}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {CARD_BLANK_STOCK_REPORT_TEXT.download}
              </Button>
            </div>
          </div>
          ...
          {report.reportError && (
            <div className="rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-[11px] text-rose-700">
              {CARD_BLANK_STOCK_REPORT_TEXT.loadError}
            </div>
          )}
...
export default CardBlankStockReportView;
