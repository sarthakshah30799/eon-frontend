...
const ReceiptItems = ({
  ...
  return <CardSection heading="Transaction Items" className="space-y-4"><div className="space-y-5">{fields.map((field, itemIndex) => {
    ...
    const product = products.find(productRecord => productRecord.id === item?.productId);
    const isMultiCurrency = String(product?.productCode ?? '').toUpperCase() === 'CM';
    const currencySource = isMultiCurrency ? cmStockingCurrencies : tradableCurrencies;
    const issuerOptions = (product ? issuers.filter(issuer => product.cardIssuerProfileIds?.includes(issuer.id)) : issuers).map(issuer => ({ value: issuer.id, label: `${issuer.code} - ${issuer.name}` }));
    const currencyOptions = currencySource.map(currency => ({ value: currency.id, label: `${currency.currencyCode} - ${currency.currencyName}` }));
    const productOptions = products.map(productRecord => ({ value: productRecord.id, label: `${productRecord.productCode} - ${productRecord.productDescription}` }));
    return <div key={field.id} className="space-y-4 rounded-md border border-border-primary p-3"><div className="overflow-x-auto"><div className="grid min-w-[1100px] grid-cols-5 gap-3"><StaticFormFieldSelect name={`items.${itemIndex}.productId`} label="Product Type" options={productOptions} disabled={readOnly} onValueChange={() => { form.setValue(`items.${itemIndex}.issuerPartyProfileId` as never, '' as never, { shouldValidate: true }); form.setValue(`items.${itemIndex}.currencyId` as never, '' as never, { shouldValidate: true }); }} /><StaticFormFieldSelect key={`currency-${itemIndex}-${product?.id ?? 'none'}-${isMultiCurrency ? 'cm' : 'cc'}`} name={`items.${itemIndex}.currencyId`} label="Currency" options={currencyOptions} disabled={readOnly || !product} /><FormFieldInput name={`items.${itemIndex}.per`} label="Per" type="number" disabled={readOnly} /><StaticFormFieldSelect key={`issuer-${itemIndex}-${product?.id ?? 'none'}`} name={`items.${itemIndex}.issuerPartyProfileId`} label="Issuer" options={issuerOptions} disabled={readOnly || !product} /><FormFieldInput name={`items.${itemIndex}.feAmount`} label="FE Amount" type="number" disabled /></div></div>{!readOnly ? <CardStockUploadSection itemIndex={itemIndex} readOnly={readOnly} issuer={issuers.find(issuer => issuer.id === item?.issuerPartyProfileId)} /> : null}<CardRows itemIndex={itemIndex} readOnly={readOnly} issuers={issuers} />{!readOnly && fields.length > 1 && <Button type="button" variant="ghost" size="sm" onClick={() => remove(itemIndex)}>Remove Item</Button>}</div>;
  })}</div>{!readOnly && <Button type="button" variant="outline" onClick={() => append(emptyItem())}>Add Item</Button>}<div className="flex justify-end border-t border-border-primary pt-3"><div className="text-sm font-semibold text-text-primary">Total FE Amount: {total}</div></div></CardSection>;
};
