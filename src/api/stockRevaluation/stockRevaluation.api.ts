...
export type StockRevaluationFrequency =
  | 'MONTHLY'
  | 'QUARTERLY'
  | 'HALF_YEARLY'
  | 'YEARLY';
export type StockRevaluationStatus = 'PENDING' | 'PROCESSED';
export interface StockRevaluationTarget {
  branchId: string;
  counterId: string;
}
...
export const stockRevaluationApi = {
  ...
  process: async (payload: {
    ...
    const response = await apiClient.postFormData<IStockRevaluation[]>(
      '/stock-revaluations/upload',
      formData
    );
    if (response.error || !response.data)
      throw new Error(response.error || 'Failed to process stock revaluation');
    return response.data;
  },

  processUploaded: async (payload: {
    targets: StockRevaluationTarget[];
    frequency: StockRevaluationFrequency;
  }) => {
    const response = await apiClient.post<IStockRevaluation[]>(
      '/stock-revaluations/process',
      {
  ...
  current: async (
    ...
    const response = await apiClient.get<IStockRevaluation[]>(
      `/stock-revaluations/current${query}`
    );
