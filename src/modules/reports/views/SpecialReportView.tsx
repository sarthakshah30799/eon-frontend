...
export const SpecialReportView = () => {
  ...
  const currentSummary = useMemo(() => {
    ...
    return {
      ...
      template:
        report.filters.templateOptions.find(
          option => option.id === (applied?.template ?? report.filters.template)
        )?.label ?? 'Account Posting',
      sortBy:
        (applied?.sortBy ?? report.filters.sortBy) ===
        ReportSortByEnum.DATE_DESC
          ? 'Date Desc'
          : 'Date Asc',
    };
  }, [
    ...
    <div className="space-y-3">
      <div className="space-y-1">
        ...
        <p className="max-w-3xl text-[11px] text-text-secondary">
          Account posting special report with branch filter and direct CSV/XLSX
          export.
        </p>
      </div>
      ...
      {report.filters.appliedFilters && (
        <div className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 text-[11px] text-text-secondary">
          ...
          {currentSummary.branches.length
            ...
            : 'All'}{' '}
          | Template {currentSummary.template} | Transaction Numbers{' '}
          {currentSummary.transactionNumbers.length
            ? currentSummary.transactionNumbers.join(', ')
            : 'All'}{' '}
          | Sort By {currentSummary.sortBy}
        </div>
      )}
