...
import { CARD_SETTLEMENT_STATUS_OPTIONS, CARD_SETTLEMENT_TEXT } from '../constants/cardSettlementConstants';
import { useCardSettlements } from '../hooks';

const label = (
  snapshot: CardStockSettlementDocument['currencySnapshot'],
  fallback: string
) => snapshot?.label ?? snapshot?.currencyCode ?? snapshot?.name ?? snapshot?.code ?? fallback;
...
export const CardSettlementListView = () => {
  ...
  const columns = useMemo<TableColumnDef<CardStockSettlementDocument>[]>(
    () => [
      { accessorKey: 'transactionNumber', header: CARD_SETTLEMENT_TEXT.transactionNumber },
      {
        accessorKey: 'kind',
      ...
      },
      {
        ...
        cell: ({ row }) => formatDateTime(row.original.transactionDate, 'DD/MM/YYYY'),
      },
      {
        id: 'issuer',
        header: CARD_SETTLEMENT_TEXT.issuer,
        cell: ({ row }) =>
          label(row.original.issuerPartyProfileSnapshot, row.original.issuerPartyProfileId),
      },
      {
        id: 'currency',
        header: CARD_SETTLEMENT_TEXT.currency,
        cell: ({ row }) => label(row.original.currencySnapshot, row.original.currencyId),
      },
      {
        id: 'branch',
        header: CARD_SETTLEMENT_TEXT.sellingBranch,
        cell: ({ row }) => label(row.original.branchSnapshot, row.original.branchId),
      },
      { accessorKey: 'itemCount', header: CARD_SETTLEMENT_TEXT.itemsCount },
      {
        ...
        cell: ({ row }) =>
          CARD_SETTLEMENT_STATUS_OPTIONS.find(option => option.value === row.original.status)?.label ??
          row.original.status,
      },
      {
        id: 'actions',
        ...
        cell: ({ row }) => (
          ...
          >
            {row.original.status === CardStockSettlementDocumentStatus.PENDING_HO_ACCEPTANCE
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-text-primary">{CARD_SETTLEMENT_TEXT.title}</h1>
          <p className="text-sm text-text-secondary">{CARD_SETTLEMENT_TEXT.description}</p>
        </div>
        <Button type="button" onClick={() => navigate('/card-settlement/create')}>
      ...
      <div className="flex flex-wrap gap-2">
        {CARD_SETTLEMENT_STATUS_OPTIONS.map(option => (
          <Button
            ...
            type="button"
            size="sm"
            variant={(filters.status ?? 'ALL') === option.value ? 'default' : 'outline'}
            onClick={() =>
              setFilters(current => ({
                ...current,
                status:
      ...
      <section className="rounded-sm border border-border-primary bg-surface-primary p-4 shadow-sm sm:p-6">
        <Table
          ...
          emptyMessage={
            query.error instanceof Error ? query.error.message : CARD_SETTLEMENT_TEXT.empty
          }
        />
      </section>
    </div>
