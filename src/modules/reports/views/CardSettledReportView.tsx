...
import { useCardSettledReport } from '../hooks';
import { ReportExportFormatEnum, ReportSortByEnum } from '../types';
import { summarizeReportSelection } from '../utils';

export const CardSettledReportView = () => {
  const { user } = useAuth();
  const report = useCardSettledReport();
  ...
  const currentSummary = useMemo(() => {
    ...
    const sortLabel =
      report.filters.sortBy === ReportSortByEnum.DATE_DESC
        ? PRODUCT_SETTLEMENT_REPORT_TEXT.dateDesc
        : PRODUCT_SETTLEMENT_REPORT_TEXT.dateAsc;
    ...
    <div className="space-y-3">
      <div className="space-y-1">
        <h1 className="text-lg font-semibold tracking-tight text-text-primary">
          {PRODUCT_SETTLEMENT_REPORT_TEXT.settledTitle}
        </h1>
        <p className="max-w-3xl text-[11px] text-text-secondary">
          {PRODUCT_SETTLEMENT_REPORT_TEXT.settledDescription}
        </p>
      </div>
      ...
      {report.filters.appliedFilters && (
        <div className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 text-[11px] text-text-secondary">
          {PRODUCT_SETTLEMENT_REPORT_TEXT.appliedPrefix}:{' '}
          {report.appliedDateRangeLabel} |{' '}
          {PRODUCT_SETTLEMENT_REPORT_TEXT.branchHeading}{' '}
          {currentSummary.branches.length
            ? currentSummary.branches.join(', ')
            : PRODUCT_SETTLEMENT_REPORT_TEXT.all}{' '}
          | {PRODUCT_SETTLEMENT_REPORT_TEXT.productHeading}{' '}
          {currentSummary.products.length
            ? currentSummary.products.join(', ')
            : PRODUCT_SETTLEMENT_REPORT_TEXT.all}{' '}
          | {PRODUCT_SETTLEMENT_REPORT_TEXT.currencyHeading}{' '}
          {currentSummary.currencies.length
            ? currentSummary.currencies.join(', ')
            : PRODUCT_SETTLEMENT_REPORT_TEXT.all}{' '}
          | {PRODUCT_SETTLEMENT_REPORT_TEXT.issuerHeading}{' '}
          {currentSummary.issuers.length
            ? currentSummary.issuers.join(', ')
            : PRODUCT_SETTLEMENT_REPORT_TEXT.all}{' '}
          | {PRODUCT_SETTLEMENT_REPORT_TEXT.sortBy} {currentSummary.sortLabel}
        </div>
      )}
        ...
        <section className="space-y-3 rounded-xl border border-border-primary bg-white p-3 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="space-y-0.5">
              <h2 className="text-sm font-semibold text-text-primary">
                {PRODUCT_SETTLEMENT_REPORT_TEXT.reportViewTitle}
              </h2>
              <p className="text-[11px] text-text-secondary">
                {PRODUCT_SETTLEMENT_REPORT_TEXT.reportViewDescription}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex items-center gap-1 rounded-full border border-slate-200 bg-slate-50 p-1">
                ...
                >
                  {PRODUCT_SETTLEMENT_REPORT_TEXT.xlsx}
                </Button>
                <Button
                  type="button"
                  size="sm"
                ...
                >
                  {PRODUCT_SETTLEMENT_REPORT_TEXT.csv}
                </Button>
              </div>
              ...
              >
                {PRODUCT_SETTLEMENT_REPORT_TEXT.download}
              </Button>
            </div>
          </div>
          ...
          {report.reportError && (
            <div className="rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-[11px] text-rose-700">
              {PRODUCT_SETTLEMENT_REPORT_TEXT.loadError}
            </div>
          )}
...
export default CardSettledReportView;
