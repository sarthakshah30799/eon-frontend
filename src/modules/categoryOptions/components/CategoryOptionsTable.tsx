...
interface CategoryOptionsTableProps {
  options: ICategoryOption[];
}

interface GroupedCategoryOptions {
  code: string;
  label: string;
  options: ICategoryOption[];
}
...
const buildGroupedOptions = (options: ICategoryOption[]): GroupedCategoryOptions[] => {
  const grouped = new Map<string, GroupedCategoryOptions>();
...
export const CategoryOptionsTable = ({
  options,
}: CategoryOptionsTableProps) => {
  ...
  if (!groupedOptions.length) {
    return (
      <div className="rounded-sm border border-dashed border-border-primary bg-surface-primary p-8 text-center text-sm text-text-secondary">
        No category options found. Create your first option.
      </div>
    );
  }
